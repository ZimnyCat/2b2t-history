...
_actions = ["%1 %v как %2 на 2b2t", "Вся правда о %1 на 2b2t", "%1 %v 24 часа на %2",
            ...
            "2b2t - эксплоит с %1", "%1 - текущее состояние %2"]
...
def gen():
    return _actions[randrange(0, len(_actions), 1)].replace("%1", _things[_rand(_things)])\
        .replace("%2", _things[_rand(_things)]).replace("%v", _verbs[_rand(_verbs)])
